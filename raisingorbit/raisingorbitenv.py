...
class RaisingOrbitEnvBase(gym.Env):
  def __init__(self):
  ...
  def reset(self):
    ...
    pointGravity = NewtonianAttraction(Constants.WGS84_EARTH_MU)
    self._propagator.addForceModel(pointGravity)
...
#     self._propagator.addForceModel(holmesFeatherstone)
    ...
    attitude = InertialProvider(FramesFactory.getEME2000().getTransformTo(self._frame, self._initial_time).getRotation())
  ...
  def render(self, mode=None):
    ...
    y3 = np.linalg.norm(self.action) if self.action != None else None
    y4 = np.arctan2(self.action[1], self.action[0]) if self.action != None else None
    y5 = np.arctan2(self.action[2], np.linalg.norm(self.action[0:1])) if self.action != None else None
...
class RaisingOrbitEnvNorm(RaisingOrbitEnvBase):
  def __init__(self):
    super().__init__()
...
class RaisingOrbitEnvNormDiscrete(RaisingOrbitEnvNorm):
  def __init__(self):
    super().__init__()
...
class RaisingOrbitEnvNormDiscreteOneAxis(RaisingOrbitEnvNormDiscrete):
  def __init__(self):
    super().__init__()
